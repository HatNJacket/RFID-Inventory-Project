...
while it fits the 431-dot width, TWO wrapped lines at font 16 when it
doesn't (the old single-line ^FB overprinted itself), every path capped
at 56 chars. Plus the barcode geometry his test prints confirmed:
33 alphanumeric chars is the printable max, 34 overflows.
...
check("no wrap block for short SKUs", "^CF0,16" not in z, z)

# ---- long SKU line: two wrapped lines at font 16 ----------------------
long_sku = "CRUX140COUNTERWEIGHTBAR-BLACK-EXTRA-LONG"
z = zpl({**BASE, "sku": long_sku})
check("long SKU wraps to two lines at font 16",
      f"^CF0,16\n^FO0,50^FB431,2,0,C^FD{long_sku}^FS" in z, z)
...
check("narrow text stays on the big line",
      "^CF0,30" in zpl({**BASE, "sku": narrow}), narrow)
check("wide text wraps sooner", "^CF0,16" in zpl({**BASE, "sku": wide}),
      wide)
...
check("case prefix + long SKU wraps",
      "^CF0,16" in z and f"^FD8 x {long_sku[:52]}" in z, z)
